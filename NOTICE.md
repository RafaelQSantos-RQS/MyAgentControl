...
- **Orphaned `skill/` (singular) tree removed (2026-08).** OAC v0.7.1 also
  ...
  (plural) — and that neither the OAC `registry.json` nor `install.sh`
  reference. Removed from `content/` by user decision; documented in the
  skills spec and `NOTICE.md`.

## Project license

`myagentcontrol` (this repository, excluding the vendored tree above) is
Copyright (c) 2026 Rafael Queiroz Santos, licensed under the **MIT License** —
see [`LICENSE`](./LICENSE).
